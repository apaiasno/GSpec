function [metric, com_x, com_y] = CoreSum(image, radius, cenxy)
% Computes core sum metric from image data for Eye Doctor Zernike
% optimization routine.
% Inputs:
% image: 2D image array of image intensity values
% radius: integer, radius of "core" in pixels
% cenxy: 1x2 array or NaN, (x,y) coordinates of initial guess for centroid
%
% Outputs:
% metric: integer, value representing core sum (negated for minimization)
...
core_mask = zeros(size(image_bgsub));
xx_com = xx - com_x;
yy_com = yy - com_y;
core_mask(xx_com.^2 + yy_com.^2 <= radius^2) = 1;
